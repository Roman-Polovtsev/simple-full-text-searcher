...
object jsonArr : ArrayType() {
    ...
    private fun toAllowedType(value: Any?): KotlinObjectMapperType =
        when (value) {
            is String -> StringType(value)
            is Number -> NumberType(value)
            is Boolean -> BooleanType(value)
            is CustomKotlinObjectMapper -> CustomKotlinObjectMapperType(value)
            is KotlinObjectMapperType -> value
            is ObjectType -> value
            is ArrayType -> value
...
fun jsonObj(block: ObjectMapperKt.() -> Unit): ObjectType = ObjectMapperKt().apply(block).objectType

@SuppressWarnings("TooManyFunctions")
class ObjectMapperKt(internal val objectType: ObjectType = ObjectType()) {
    ...
    infix fun String.to(value: CustomKotlinObjectMapper) {
        objectType.values[this] = CustomKotlinObjectMapperType(value)
    }

    infix fun String.to(value: KotlinObjectMapperType) {
    ...
    @Suppress("UNUSED_PARAMETER")
    infix fun String.to(value: ObjectMapperKt): Nothing =
 ...
 */
fun unescapedJson(validJson: String?): RawJsonType = RawJsonType(validJson)

interface CustomKotlinObjectMapper {
    fun serialize(): String
}

sealed class KotlinObjectMapperType {
    internal abstract fun prettyPrint(level: Int, spaces: Int): String
}

private data class StringType(val value: String) : KotlinObjectMapperType() {
    override fun toString(): String = value.quotedEscaped()
    override fun prettyPrint(level: Int, spaces: Int): String = toString()
}

private data class NumberType(val value: Number) : KotlinObjectMapperType() {
    override fun toString(): String = value.toString()
    override fun prettyPrint(level: Int, spaces: Int): String = toString()
}

private data class BooleanType(val value: Boolean) : KotlinObjectMapperType() {
    override fun toString(): String = value.toString()
    override fun prettyPrint(level: Int, spaces: Int): String = toString()
}

private data class CustomKotlinObjectMapperType(val value: CustomKotlinObjectMapper) : KotlinObjectMapperType() {
    override fun toString(): String = value.serialize().quotedEscaped()
    override fun prettyPrint(level: Int, spaces: Int): String = toString()
}

data class ObjectType(internal val values: MutableMap<String, KotlinObjectMapperType> = mutableMapOf()) :
    KotlinObjectMapperType() {
...
open class ArrayType(private val values: List<KotlinObjectMapperType> = emptyList()) : KotlinObjectMapperType() {
...
data class RawJsonType(val value: String?) : KotlinObjectMapperType() {
...
private object NullType : KotlinObjectMapperType() {
    override fun toString(): String = NULL_PRINT
    override fun prettyPrint(level: Int, spaces: Int): String = toString()
}

private data class DefaultType(val value: Any) : KotlinObjectMapperType() {
